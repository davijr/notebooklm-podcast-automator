...
def main():
    ...
    **Note:** You must have Chrome running with remote debugging enabled.
    ...
    with st.sidebar:
        ...
        st.code(f"/Applications/Google\\ Chrome.app/Contents/MacOS/Google\\ Chrome --remote-debugging-port={port}", language="bash")
        st.markdown("**Note:** Adjust the path for your operating system.")
            ...
            try:
                ...
                with NotebookLMAutomator(port) as automator:
                    try:
                        ...
                        print(f"Example command to launch Chrome: /Applications/Google\\ Chrome.app/Contents/MacOS/Google\\ Chrome --remote-debugging-port={port}")
                        log_output.text(log_capture.getvalue())
